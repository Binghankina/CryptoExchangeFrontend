<template>
  <div class="order-main">
    <div
class="order-head" v-text="$t('orders.history.title')"
/>
...
export default {
  ...
  mounted() {
    ...
    this.getTradeStatus = getTradeStatus();
    this.getData();
  ...
  methods: {
    ...
    vaildStatus(status) {
      if (status === "Waiting" || status === "未完成") {
        return "wait";
      } else if (status === "Filled" || status === "已完成") {
        return "done";
      } else if (status === "Canceled" || status === "已完成") {
        return "cancel";
      } else {
        return "";
      }
    ...
    getData($page = null) {
      ...
      const payload = {
        ...
        state: this.vaildStatus(this.status),
