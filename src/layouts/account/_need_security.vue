<template>
  <div>
    ...
    <div class="desc">Select an authentication method from the list below for your account’s safety.</div>
    <div class="box">
      <p v-for="(data, index) in actions" :key="index" @click="actionChange(index)">
        <img :src="data.logo" class="icon" />
        <span class="name">{{ data.name }}</span>
        <span class="action" :class="{ selected: selected === index }"></span>
      </p>
    </div>
    <button
      type="submit"
    ...
    <div class="later-footer">
      <span @click="$emit('closeModal')">Maybe later</span>
    </div>
  </div>
</template>
