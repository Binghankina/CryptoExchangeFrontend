...
export default {
  ...
  computed: {
    ...
    datePanelLabel() {
      ...
      const datePanelLabel = this.$t("datepicker.datePanelLabel");
  ...
  watch: {
    value(newVal) {
      ...
      this.panelDate =
        this.startDate ||
        (this.multiple ? this.dates[this.dates.length - 1] : this.dates[0]) ||
        new Date();
    },
    ...
    focusedDate(date) {
      const isDifferentYear =
        date.getFullYear() !== this.panelDate.getFullYear();
      const isDifferentMonth =
        isDifferentYear || date.getMonth() !== this.panelDate.getMonth();
