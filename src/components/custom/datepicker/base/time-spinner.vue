  ...
  <div :class="classes">
    <div :class="[prefixCls+ '-list']" ref="hours">
      <ul :class="[prefixCls + '-ul']">
        <li
          ...
          v-for="(item, index1) in hoursList"
          :key="index1"
          v-show="!item.hide"
          @click="handleClick('hours', item)"
        >{{ formatTime(item.text) }}</li>
      </ul>
    </div>
    <div :class="[prefixCls+ '-list']" ref="minutes">
      <ul :class="[prefixCls + '-ul']">
        <li
          :class="getCellCls(item)"
          v-for="(item, index2) in minutesList"
          :key="index2"
          v-show="!item.hide"
    ...
    <div :class="[prefixCls+ '-list']" v-show="showSeconds" ref="seconds">
      <ul :class="[prefixCls + '-ul']">
        <li
          :class="getCellCls(item)"
          v-for="(item, index3) in secondsList"
          :key="index3"
          v-show="!item.hide"
...
import { deepCopy, scrollTop, firstUpperCase } from "iview/src/utils/assist";
  ...
  computed: {
    ...
    minutesList() {
      ...
      for (let i = 0; i < 60; i += step) {
        ...
        if (
          this.disabledMinutes.length &&
          this.disabledMinutes.indexOf(i) > -1
        ) {
    ...
    secondsList() {
      ...
      for (let i = 0; i < 60; i += step) {
        ...
        if (
          this.disabledSeconds.length &&
          this.disabledSeconds.indexOf(i) > -1
        ) {
  ...
  watch: {
    ...
    focusedTime(updated, old) {
      timeParts.forEach((part, i) => {
        ...
        const valueIndex = this[`${part}List`].findIndex(
          obj => obj.text === updated[i]
        );
