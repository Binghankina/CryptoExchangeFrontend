  ...
  >
    <div ref="reference" :class="[prefixCls + '-rel']">
      <slot>
        <div
          class="m"
          @mouseover="handleInputMouseenter"
        ...
        >
          ...
          <Input
            ...
            :name="name"
          />
          <i v-if="showClose" class="ivu-icon ivu-icon-ios-close-circle" @click="handleClear()"/>
        </div>
      </slot>
    </div>
...
export default {
  ...
  computed: {
    ...
    visualValue() {
      const data = this.formatDate(this.internalValue).split(" - ");
  ...
  methods: {
    ...
    handleKeydown(e) {
      ...
      if (keyCode === 13) {
        ...
        if (timePickers.length > 0) {
          ...
          const pickerIndex = Math.floor(
            this.focusedTime.column / columnsPerPicker
          );
    ...
    navigateTimePanel(direction) {
      ...
      const maxNrOfColumns =
        (timePickers[0].showSeconds ? 3 : 2) * timePickers.length;
      const column = (currentColumn => {
        const incremented =
          currentColumn + (horizontal ? (direction === "left" ? -1 : 1) : 0);
      ...
      if (vertical) {
        ...
        const pickerPossibleValues =
          timePickers[pickerIndex][`${timeParts[col]}List`];
        const nextIndex =
          pickerPossibleValues.findIndex(
            ({ text }) => this.focusedTime.time[pickerIndex][col] === text
          ) + increment;
        const nextValue =
          pickerPossibleValues[nextIndex % pickerPossibleValues.length].text;
    ...
    navigateDatePanel(direction, shift) {
        ...
        if (this.type === "year") {
          this.focusedDate = new Date(
            this.focusedDate.getFullYear() +
              mapPossibleValues(direction, 0, 10),
      ...
      if (this.type.match(/^date/)) {
        const lastOfMonth = getDayCountOfMonth(
          initialDate.getFullYear(),
          initialDate.getMonth()
        );
        const startDay = initialDate.getDate();
        const nextDay =
          focusedDate.getDate() + mapPossibleValues(direction, 1, 7);
        ...
        if (nextDay < 1) {
          if (direction.match(/left|right/)) {
            ...
            focusedDate.setDate(
              startDay + Math.floor((lastOfMonth - startDay) / 7) * 7
            );
          }
      ...
      if (this.type.match(/^month/)) {
        focusedDate.setMonth(
          focusedDate.getMonth() + mapPossibleValues(direction, 1, 3)
        );
      }

      if (this.type.match(/^year/)) {
        focusedDate.setFullYear(
          focusedDate.getFullYear() + mapPossibleValues(direction, 1, 3)
        );
      }
    ...
    handleInputMouseenter() {
      ...
      if (this.visualValue[0] && this.visualValue[1] && this.clearable) {
        this.showClose = true;
      }
    },
