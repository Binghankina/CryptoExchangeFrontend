<template>
  <div id="orders_open" class="table-div">
    <dt>
      ...
      <span class="text-center action">{{ $t('table.cancel') }}</span>
    </dt>
    ...
    <dd v-else-if="$store.getters['exchange/getControlOrderArray'].length" class="all">
      <p v-for="data of $store.getters['exchange/getControlOrderArray']" :key="data.id">
        ...
        <span class="text-center action">
          <Icon type="md-close" @click="cancelOrder(data.id)" />
        </span>
      </p>
    </dd>
    <modal-empty v-else :first="$t('orders.open.empty')" />
...
export default {
  ...
  mounted() {
    if (this.$store.getters["user/isLoggedIn"])
      this.$store.dispatch("exchange/getOrdersOpen");
  },
  methods: {
